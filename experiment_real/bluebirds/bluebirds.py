...
'''
f = open("./experiment_real/bluebirds/gt.yaml", "r")
...
for i, k in task_label.items():
    ...
    G = np.append(G, [int(k)])
f.close()
pd.DataFrame(G).to_csv("./experiment_real/bluebirds/gt.csv")


f = open("./experiment_real/bluebirds/labels.yaml", "r")
...
pd.DataFrame(task_worker_label).to_csv("./experiment_real/bluebirds/twl.csv")
'''
...
task_worker_label = np.array(pd.read_csv("./experiment_real/bluebirds/twl.csv")[['0', '1', '2']].values.tolist())
...
data.to_csv("./experiment_real/bluebirds/L1_m.csv")
...
plt.savefig("./experiment_real/bluebirds/bird.pdf")
